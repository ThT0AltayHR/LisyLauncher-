...
fun LaunchGameOperation(
    ...
    //部分模组（语音聊天类）运行时需要麦克风权限，在此注册请求回调
    //权限弹窗为系统异步弹窗，不会阻塞后续的游戏启动流程
    ...
    when (val operation = launchGameOperation) {
        ...
        is LaunchGameOperation.RequestMicrophonePermission -> {
            LaunchedEffect(Unit) {
                val version = operation.version
                val quickPlay = operation.quickPlay
                withContext(Dispatchers.Main) {
                ...
                //不等待用户对系统权限弹窗的响应，直接继续启动流程
                //授权结果会在模组实际尝试使用麦克风时生效（游戏加载通常比权限弹窗响应耗时更长）
                launchGameViewModel.updateOperation(LaunchGameOperation.RealLaunch(version, quickPlay))
            }
        }
        ...
        is LaunchGameOperation.TryLaunch -> {
            LaunchedEffect(Unit) {
                ...
                //部分模组（如语音聊天类模组）需要访问麦克风，在此确保已申请该权限
                //即使用户拒绝授权，也不阻塞本次启动，仅对应的模组功能将不可用
                if (
                    ContextCompat.checkSelfPermission(activity, Manifest.permission.RECORD_AUDIO)
                    != PackageManager.PERMISSION_GRANTED
                ) {
                    launchGameViewModel.updateOperation(LaunchGameOperation.RequestMicrophonePermission(version, quickPlay))
                    return@LaunchedEffect
                }
