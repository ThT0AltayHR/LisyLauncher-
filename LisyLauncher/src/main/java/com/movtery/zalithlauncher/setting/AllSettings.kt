...
object AllSettings : SettingsRegistry() {
     ...
     * 手势控制
     */
    val gestureControl = boolSetting("gestureControl", true)

    /**
     * 手势控制点击时触发的鼠标按钮
     */
    val gestureTapMouseAction = enumSetting("gestureTapMouseAction", GestureActionType.MOUSE_LEFT)

    /**
     * 手势控制长按时触发的鼠标按钮
     */
    val gestureLongPressMouseAction = enumSetting("gestureLongPressMouseAction", GestureActionType.MOUSE_RIGHT)
     ...
     * 更新检查是否包含测试版（预发布）版本
     */
    val showBetaUpdates = boolSetting("showBetaUpdates", false)
